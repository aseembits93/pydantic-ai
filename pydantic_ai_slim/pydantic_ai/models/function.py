...
def _estimate_string_tokens(content: str | Sequence[UserContent]) -> int:
    ...
    if isinstance(content, str):
        return len(_token_split_pattern.split(content.strip()))
    # Use sum and avoid redundant += 0
    tokens = 0
    for part in content:
        if isinstance(part, str):
            tokens += len(_token_split_pattern.split(part.strip()))
        elif isinstance(part, BinaryContent):
            tokens += len(part.data)
        # For AudioUrl or ImageUrl, do nothing (tokens += 0)
    return tokens
